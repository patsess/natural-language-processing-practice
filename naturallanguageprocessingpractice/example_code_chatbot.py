...
if __name__ == '__main__':
    ...
    print("Predicted {0} correctly out of {1} test examples"
          .format(n_correct, len(y_test)))
    ...
    # Define a respond function, taking the message and existing params as
    # input
    ...
    def negated_ents(phrase):
        ...
        for chunk in chunks:
            for ent in ents:
                if ent in chunk:
                    # If the entity contains a negation, assign the key to be
                    # False
    ...
    policy_rules = {
        (INIT, "ask_explanation"):
            (INIT, "I'm a bot to help you order coffee beans"),
        (INIT, "order"): (CHOOSE_COFFEE, "ok, Colombian or Kenyan?"),
        (CHOOSE_COFFEE, "specify_coffee"):
            (ORDERED, "perfect, the beans are on their way!"),
        (CHOOSE_COFFEE, "ask_explanation"):
            (CHOOSE_COFFEE, "We have two kinds of coffee beans - the Kenyan "
                            "ones make a slightly sweeter coffee, and cost "
                            "$6. The Brazilian beans make a nutty coffee and "
                            "cost $5.")
    }
    ...
    for message in ["I want a mid range hotel", "no that doesn't work for me"]:
        print("USER: {}".format(message))
        response, params, suggestions, excluded = respond(
            message, params, suggestions, excluded)
        print("BOT: {}".format(response))
    ...
    def policy(intent):
        ...
        if intent == "deny":
            return "Ok", None
        if intent == "order":
            return ("Unfortunately, the Kenyan coffee is currently out of "
                    "stock, would you like to order the Brazilian beans?",
    ...
    policy_rules = {
        (INIT, "order"):
            (INIT, "you'll have to log in first, what's your phone number?",
             AUTHED),
        (INIT, "number"): (AUTHED, "perfect, welcome back!", None),
        (AUTHED, "order"):
            (CHOOSE_COFFEE, "would you like Colombian or Kenyan?", None),
        (CHOOSE_COFFEE, "specify_coffee"):
            (ORDERED, "perfect, the beans are on their way!", None)
    }
